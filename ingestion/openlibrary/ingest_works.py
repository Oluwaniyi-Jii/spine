...
def run_ingest_works(filepath: str = None, limit: int = LIMIT_ROWS):
    if not filepath:
        filepath = os.path.join(DATA_DIR, "openlibrary", "ol_dump_works.txt.gz")
    
    if not os.path.exists(filepath):
        logger.warning(f"Dump file not found: {filepath}. Skipping works ingestion.")
        return

    batch_id = start_ingestion_batch("openlibrary", os.path.basename(filepath))
    total_processed = 0
    total_inserted = 0
    ...
    try:
        for chunk in parse_openlibrary_dump_chunks(filepath, chunk_size=20000, limit_rows=limit):
            ...
            for item in chunk:
                total_processed += 1
                if item["type"] in ("/type/work", "work"):
